...
def _render_alignment_md(hub: str, audit: dict) -> str:
    """Render the align-principles audit as a hub-level ALIGNMENT.md.

    Markdown is the durable, grep-friendly, GitHub-rendered form — no
    HTML, no JS, no images. Hub is the page title; the summary leads;
    each principle gets a present/gap table; per-repo totals are a
    pinned list at the bottom. Members are sorted with the hub repo
    first to match the audit payload's ordering.
    """
    members = audit.get("members") or []
    if hub in members:
        members = [hub] + [m for m in members if m != hub]
    principles = audit.get("principles") or []
    gap_by_repo = audit.get("gap_by_repo") or {}
    summary = audit.get("summary") or ""

    lines: list[str] = [f"# {hub} — Design-principles alignment", ""]
    if summary:
        lines += [summary, ""]
    lines += [
        "Generated by git-suite's Step 7 audit (`POST /api/order/{sid}/{hub}/align-docs`).",
        "A principle is **present** when the audit marked the repo as such; **gap** means",
        "the repo is missing or below the hub's baseline and is a candidate to fix.",
        "",
    ]
    for p in principles:
        name = p.get("name", "(unnamed)")
        repos = p.get("repos") or []
        by_repo = {r.get("repo"): r for r in repos if isinstance(r, dict)}
        lines.append(f"## {name}")
        lines.append("")
        if not members:
            lines += ["_(no members audited)_", ""]
            continue
        lines.append("| Repository | Status | Note |")
        lines.append("|---|---|---|")
        for m in members:
            r = by_repo.get(m)
            if r is None:
                status, note = "—", "_(not in audit)_"
            elif r.get("present"):
                status, note = "✅ present", (r.get("note") or "").strip() or "—"
            else:
                status, note = "❌ gap", (r.get("note") or "").strip() or "—"
            lines.append(f"| `{m}` | {status} | {note} |")
        lines.append("")
    if gap_by_repo:
        lines += ["## Per-repo gap totals", ""]
        ordered = sorted(gap_by_repo.items(), key=lambda kv: (-kv[1], kv[0]))
        for repo, gaps in ordered:
            tag = " _(hub)_" if repo == hub else ""
            lines.append(f"- `{repo}`{tag}: {gaps} gap{'' if gaps == 1 else 's'}")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"


async def push_alignment(token: str, owner: str, hub: str, audit: dict) -> dict:
    """Render the audit to ALIGNMENT.md and push it to the hub repo.

    Idempotent: if the existing file already matches the rendered content
    (byte-for-byte), we skip the network round-trip entirely and return
    `pushed: False, reason: "up to date"`. Otherwise we update via the
    Contents API with the existing SHA so we never silently overwrite a
    concurrent edit.
    """
    content = _render_alignment_md(hub, audit)
    existing = await get_file(token, owner, hub, "ALIGNMENT.md")
    if existing is not None and existing["content"] == content:
        log.info("ALIGNMENT.md unchanged for %s — skipping push", hub)
        return {"pushed": False, "hub": hub, "reason": "up to date",
                "path": "ALIGNMENT.md"}
    sha = None if existing is None else existing["sha"]
    content_b64 = base64.b64encode(content.encode("utf-8")).decode("ascii")
    await push_file(token=token, owner=owner, repo=hub, path="ALIGNMENT.md",
                    content_b64=content_b64,
                    message=f"chore: refresh ALIGNMENT.md [{hub}]", sha=sha)
    log.info("ALIGNMENT.md pushed for %s (sha_was=%s)", hub, sha)
    return {"pushed": True, "hub": hub, "sha_was": sha,
            "path": "ALIGNMENT.md"}
